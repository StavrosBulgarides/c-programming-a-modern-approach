...
int main(void)
{
    ...
    for (day = 0; day < 30; day++) {                                //iterate through days
        ...
        for (hour = 0; hour < 24; hour++) {                         //iterate through hours within days
            day_average += temperature_readings[day][hour];
        }

        day_average = (!day_average == 0.0f ? day_average /= 24 : day_average);  //need to dive into this as I woulnd't have done it this way
    ...
    printf("\nAverage monthly temperature: %.0f", month_average );

    return 0;
}
//interestingly this gets compiler warnings
